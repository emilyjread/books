...
def mainpage():
	if 'userid' in session:
		...
		mysql=connectToMySQL("favorite_books")
		query=(f"select books.id, uploader_id, username, title from books JOIN users on books.uploader_id= users.id")
		books=mysql.query_db(query)
		

		mysql=connectToMySQL("favorite_books")
		query=(f"select title, books.id, uploader_id, username, favorites.users_id, favorites.books_id from favorites JOIN users on favorites.users_id= users.id JOIN books ON books.id= favorites.books_id WHERE favorites.users_id={session['userid']}")
...
def showbook(id):
		...
		if books[0]['uploader_id']== session["userid"]:
			...
			return render_template("showbook.html", user=user, books= books, fans=fans)
